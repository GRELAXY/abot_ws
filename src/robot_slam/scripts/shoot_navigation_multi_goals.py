...
#	rospy.init_node('object_position_node',anonymous=True)
	...
	#print("move_flog")
	#print(move_flog)
	if (abs(flog1) >= 10 and find_id_sh == 9 and move_flog==0):
		#movefortheta(flog0)
		msg = Twist()
		msg.angular.z = -0.02*flog0
		self.pub.publish(msg)
		rospy.sleep(0.1)
	elif (abs(flog1) >= 5 and abs(flog1) < 10 and find_id_sh == 9 and move_flog==0):
		msg = Twist()
		msg.angular.z = -0.023*flog0
		self.pub.publish(msg)
		rospy.sleep(0.1)
	elif (abs(flog1) >= 2 and abs(flog1) < 5 and find_id_sh == 9 and move_flog==0):
		msg = Twist()
		msg.angular.z = -0.03*flog0
		self.pub.publish(msg)
		rospy.sleep(0.1)
	elif (abs(flog1) <= 2 and find_id_sh == 9 and move_flog==0):
		print "ok"
	...
	for marker in data.markers:
		if (marker.id==goal_2 and place == 2):
			ar_x=marker.pose.pose.position.x+0.165
			ar_x_abs=abs(ar_x)
...
def movefor2y():
	...
	twist_msg.angular.z=0.0
	start_time=rospy.Time.now()
	duration = Duration.from_sec(3.7)
	while rospy.Time.now() - start_time<duration:
		pub.publish(twist_msg)
		rospy.sleep(0.1)
...
def move2endfory7():
	...
	twist_msg.angular.z=0.0
	start_time=rospy.Time.now()
	duration = Duration.from_sec(2.6)
	while rospy.Time.now() - start_time<duration:
		pub.publish(twist_msg)
		rospy.sleep(0.1)
...
def move2endforx7():
	...
	twist_msg.angular.z=0.0
	start_time=rospy.Time.now()
	duration = Duration.from_sec(2.7)
	while rospy.Time.now() - start_time<duration:
		pub.publish(twist_msg)
		rospy.sleep(0.1)
